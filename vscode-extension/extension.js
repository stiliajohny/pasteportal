...
function encrypt(password, text) {
  ...
  try {
    ...
    if (password.length < passwordLengthMin) throw new Error('Password should be at least 8 characters long.');
    if (password.length > passwordLengthMax) throw new Error('Password should be less than 30 characters long.');
    if (password.includes(' ')) throw new Error('Password should not contain spaces.');
    if (password.includes('\t')) throw new Error('Password should not contain tabs.');
    if (password.includes('\n')) throw new Error('Password should not contain new lines.');
    if (password.includes('\r')) throw new Error('Password should not contain carriage returns.');
    if (password.includes('\v')) throw new Error('Password should not contain vertical tabs.');
    if (password.includes('\f')) throw new Error('Password should not contain form feeds.');
    if (password.includes('\0')) throw new Error('Password should not contain null characters.');
...
function multiLineClipboard(id, password) {
  ...
  try {
    ...
    if (password) {
      if (typeof password !== 'string') throw new TypeError('Password must be a string');
      if (password.length < passwordLengthMin) throw new Error('Password cannot be empty');
...
function activate(context) {
  ...
  let get_encrypted_paste = vscode.commands.registerCommand('pasteportal.get-encrypted-paste', async function () {
    // check if user has accepted the service agreement or not
    if (!(await checkServiceAgreement())) {
      return;
    }
  ...
  let store_encrypted_paste = vscode.commands.registerCommand('pasteportal.store-encrypted-paste', async function () {
    try {
      // check if user has accepted the service agreement or not
      if (!(await checkServiceAgreement())) {
        return;
